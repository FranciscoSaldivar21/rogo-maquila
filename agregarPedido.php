...
<html>
    <head>
        ...
        <script>
            ...
            function imprimeCarrito(){
                total2 = 0;
                document.getElementById('tabla').innerHTML = "<thead><tr><th>ID MATERIAL</th><th>Cantidad</th><th>Precio unitario</th><th>Total</th></tr></thead>";
                productos.forEach((producto) => {
                    total = parseInt(producto.cantidad) * parseInt(producto.precio);
                    total2 += total;
            ...
            function validaCliente(idProveedor){
                $.ajax({
                        ...
                        success : function(bandera){
                            if(bandera){
                                ...
                                $('#mensaje').html('Error, el proveedor con id '+idProveedor+' no existe').show(); //Permite que se vuelva a mostrar despues de ser ocultado
            ...
            function validaProducto(codigo){
                proveedor = document.getElementById('idCliente').val;
                $.ajax({
                        url : 'verificaMaterial.php',
                        type : 'post',
                        dataType : 'JSON', //Tipo de dato que recibe
                        data : 'codigo='+codigo+'idProveedor'+proveedor,
                        success : function(data){
                            if(data.bandera){
                                ...
                                $("#precio").val('');
                                $("#descripcion").val('');
                                $('#mensaje').html('Error, el producto con codigo '+codigo+' no existe para este proveedor').show(); //Permite que se vuelva a mostrar despues de ser ocultado
                                //$('#validacion').slideUp(5000); //oculta de abajo hacia arriba
                                setTimeout(function(){ //INICIO UN RELOJ 
                                    $('#mensaje').html('').show();//OCULTO EL MENSAJE
                                },5000);
                ...
                $.ajax({
                    ...
                    success : function(data){
                        if(data.bandera){
                            productos.forEach((producto) => {
                                $.ajax({
                                    url : 'agregarProdPedido.php',
                                    type : 'post',
                                    dataType : 'text', //Tipo de dato que recibe
                                    data : {idVenta : data.idPedido, idProducto : producto.id, cantidad : producto.cantidad, precio : producto.precio},
                                    success : function(bandera){
                                        if(bandera){
            ...
            function abrirClientes(){
                ...
                ventana = window.open("tablaProveedor.php","Proveedor",strCaracteristicasVentana);
            }
            function abrirProductos(){
                proveedor = document.getElementById('idCliente');
                strCaracteristicasVentana = "menubar=false,location=false,resizable=false,scrollbars=no,status=no,width=800px,height=400,right=400px";
                ventana = window.open("tablaMaterial.php","Material",strCaracteristicasVentana);
            }
        </script>
        <style>
            input{
    ...
    <body>
        ...
        <div id = "datosCliente">
            <form name = "forma01">
                <label for = "idCliente">ID Proveedor <input type = "text" id = "idCliente" onBlur = "validaCliente(value);"></label>
                <input id = "botonClientes" style = "padding: 5px;"value = "Mostrar Proveedores" type = "submit" onclick = "abrirClientes();">
                <label for = "cliente">Nombre del proveedor <input disabled type = "text" id = "nombre"></label>
            </form>
        </div>
        <div style = "display:none;" id = "productoVenta">
            ...
            <input id = "botonProductos" style = "padding: 5px;"value = "Mostrar materiales" type = "submit" onclick = "abrirProductos();">
        </div>
